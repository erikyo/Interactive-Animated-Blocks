...
function navigator(entry) {
  ...
  if (children) {
    const header = document.getElementById('navigator-header') || document.createElement('div');
    const navChildrens = {
      labels: [],
      data: []
    };
    ...
    headerProgress.id = 'navigator-progress';
    Object.values(children).forEach((child, key) => {
      const c = {};
      const lastScrollPosition = window.scrollY;
      c.id = child.id ? child.id : key;
      c.slug = 'child-' + c.id;
      c.title = child.id || 'child-' + key;
      c.scrollY = lastScrollPosition + child.getBoundingClientRect().top;
      const labelContainer = document.createElement('p');
      labelContainer.classList.add('label-' + c.id);
      labelContainer.textContent = c.title;
      navChildrens.data[key] = c;
      headerLabels.append(labelContainer);
      const dot = document.createElement('div');
      dot.classList.add('dot-' + c.id);
      headerProgress.append(dot);
    });
