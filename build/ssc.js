...
/* harmony import */ var _style_ssc_scss__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./style/ssc.scss */ "./src/style/ssc.scss");
...
class _ssc {
  constructor(options) {
    ...
    (0,_babel_runtime_helpers_defineProperty__WEBPACK_IMPORTED_MODULE_0__["default"])(this, "screenControl", (entries, observer) => {
      ...
      entries.forEach(entry => {
        ...
        entry.target.action = entry.isIntersecting !== entry.target.dataset.visible ? entry.isIntersecting ? 'enter' : 'leave' : ''; // is colliding with borders
    ...
    (0,_babel_runtime_helpers_defineProperty__WEBPACK_IMPORTED_MODULE_0__["default"])(this, "scrollJacking", entry => {
    ...
    this.animations = [];
    this.staggerPresets = _utils_data__WEBPACK_IMPORTED_MODULE_2__.textStaggerPresets; // avoid scrolling before the page has been loaded
  ...
  textStagger(entry) {
    ...
    if (item.action === 'enter' && this.checkVisibility(entry.target, 'between', 25)) {
      ...
      const replaced = item.lastChild.textContent.replace(splitByRegex, `<span class="${item.sscItemOpts.splitBy}">$&</span>`);
      ...
      const anim = animejs_lib_anime_es_js__WEBPACK_IMPORTED_MODULE_1__["default"].timeline({
        loop: false,
        autoplay: false
      });
      this.staggerPresets[preset].forEach((data, index) => {
        switch (index) {
          case 0:
            anim.add({ ..._utils_data__WEBPACK_IMPORTED_MODULE_2__.textStaggerPresets[preset][index],
          ...
          case 1:
            anim.add({ ...(_utils_data__WEBPACK_IMPORTED_MODULE_2__.textStaggerPresets[preset][index] || null),
              targets: item,
              easing,
              duration,
              delay: duration,
  ...
  handleVideo360(e) {
    ...
    function changeAngle() {
      ...
      if (video.readyState > 2) {
        video.currentTime = (e.clientX - rect.left) / rect.width * video.duration * video.spinRatio;
      }
    }
