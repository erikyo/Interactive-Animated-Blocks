...
import { actionsTemplate, seqActionTemplate } from '../utils/data';
import type {
	ActionRowProps,
	Label,
	SSCAnimationSceneData,
	SSCAnimationSceneDataExperimental,
} from '../types.d.ts';
...
//--------------------------------------------------------------------------------------------------------------
function ActionRowExperimental(props: ActionRowProps) {
	const { attributes, listeners, setNodeRef, transform, transition } =
		useSortable({ id: props.id });
...
export function ActionListExperimental(props: {
	data: SSCAnimationSceneDataExperimental[];
	onSave: (data: SSCAnimationSceneDataExperimental[]) => void;
}): JSX.Element {
	const [animationProps, setAnimationProps] = useState(props.data);
	 ...
	 * @param {string} label    - The property name in each item of the input array to be used as the label for the option.
	 * @param {string} value    - The property name in each item of the input array to be used as the value for the option.
	 * @param {number} duration -asdsadd
	 * @return {SSCAnimationSceneDataExperimental[]} An array of SSCAnimationTypeDef objects, where each object has a label and value property.
	 */
	const provideSelectOptions = (
		array: any[],
		label: string,
	...
	function handleDragEnd(event: { active: any; over: any }) {
		...
		const newItems = (items: SSCAnimationSceneDataExperimental[]) => {
			const oldIndex = items.map((o) => o.id).indexOf(active.id);
			const newIndex = items.map((x) => x.id).indexOf(over.id);
			return arrayMove(items, oldIndex, newIndex);
		};
	...
	const addAction = (): void => {
		const newID: number = animationProps.length
			? Math.max(...animationProps.map((x) => x.id)) + 1
			: 1;
		const newAnimationProps: SSCAnimationSceneDataExperimental = {
			id: newID,
			key: newID,
			action: seqActionTemplate[1].action,
			value: seqActionTemplate[1].valueDefault,
			duration: 1000,
		};
		setAnimationProps([...animationProps, newAnimationProps]);
		props.onSave(animationProps);
	...
	function handleChange(ev: string, data: DndChangedProps) {
		...
		if (data.changed === 'action') {
			...
			newAnimationProps[selectedItem].value = ev;
		} else if (data.changed === 'duration') {
			newAnimationProps[selectedItem].duration = ev;
		}
		setAnimationProps([...newAnimationProps]);
		props.onSave(newAnimationProps);
	}
	...
	return (
		<>
			<h5>Action Sequence Experimental</h5>
			<section
				className={'action-sequence'}
				style={{ margin: '24px 0', position: 'relative' }}
			>
